...
const createTableRow = (question) => {
  ...
  questionTitle.innerHTML = `
    <div class="question-title" onclick="toggleDescription('${getSlugFromQuestion(
      question
    )}')"> ${question.title} </div>
    <div class="question-description-${getSlugFromQuestion(question)}"> ${
  ...
  deleteButton.classList.add(
    `delete-button-for-${getSlugFromQuestion(question)}`
  );
